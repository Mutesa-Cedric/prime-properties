...
const Gallery = ({ propertyImages }: GalleryProps) => {
    return (
        <div className="w-full">
            ...
            <main className="flex justify-between items-center lg:px-60 py-20">
                {
                    propertyImages.map((img, i) => (
                        <div key={i} className="relative w-80 h-80 group flex items-center justify-center cursor-pointer">
                            <Image src={img.bannerImage} layout="fill" objectFit="cover" className="shadow-md rounded bg-[#C4C4C4]" />
                            <div className="hidden group-hover:flex absolute w-[85%] flex items-center justify-center h-[85%] bg-primary-light opacity-90 rounded">
